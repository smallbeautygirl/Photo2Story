...
- **RQ3：** 跨照片因果敘事推斷步驟是否提升故事連貫性（有 vs 無 causal inference 步驟）？
...
**兩步驟生成：**
1. **因果推斷（Causal Inference）：** 先讓 LLM 推斷各照片之間的因果關係與時間流向（例如：「第1張是出發，第2張是抵達，第3張是活動高潮」），生成隱性敘事骨架
2. **故事生成：** 以敘事骨架 + 照片描述 + 情境描述為輸入，生成有轉折的連貫故事文字

**Prompt 結構：**
```
[步驟1] 以下是依序排好的 {K} 張照片描述，
        請推斷它們之間的因果關係與故事發展脈絡（一段話）。
[照片描述] 第1張：{描述1} / ...

[步驟2] 根據上述敘事脈絡與旅遊情境，
        生成 {K} 頁繪本故事，每頁約 2–3 句，風格：{風格}。
[情境] {使用者輸入}
```

**論文貢獻定位：** 現有方法（VIST、AREL）直接從照片描述拼接成故事，缺乏跨照片因果推斷步驟，導致故事轉折生硬。本方法顯式建模照片間關係後再生成，可提升敘事連貫性。

**參考論文：** AR-LDM (Pan et al., NeurIPS 2022)、AREL (Wang et al., EMNLP 2018)、Make-A-Story (Rahman et al., CVPR 2023)
...
| `ablation_rq3_no_causal` | 混合 | 有 | 無因果推斷（直接生成） | RQ3-A |
| `ablation_rq3_causal` | 混合 | 有 | 有因果推斷（本方法） | **RQ3 主張** |
